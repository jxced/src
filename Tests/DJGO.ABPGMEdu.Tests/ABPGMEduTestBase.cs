...
namespace DJGO.ABPGMEdu.Tests
{
    public abstract class ABPGMEduTestBase : AbpIntegratedTestBase<ABPGMEduTestModule>
    {
        ...
        protected void UsingDbContext(Action<ABPGMEduDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ABPGMEduDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ABPGMEduDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ABPGMEduDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ABPGMEduDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPGMEduDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ABPGMEduDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ABPGMEduDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ABPGMEduDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ABPGMEduDbContext, Task<T>> func)
        {
            T result;
